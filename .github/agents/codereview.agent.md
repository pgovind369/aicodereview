...
## Purpose
Comprehensive automated code review that analyzes git changes and coordinates parallel execution of specialized review agents for security, OWASP vulnerabilities, bugs, code quality, and performance issues.
...
When invoked (via `@codereview` in Copilot chat), follow this orchestration workflow:
...
1. **Identify changed files**:
   ...
   git status --short
   ```
   Look for: Modified (M), Added (A), Renamed (R), and untracked (??) files

2. **Extract diffs for each changed file**:
   ```bash
   git diff HEAD <file>
   ```
   For new files, get the entire content since there's no previous version
...
- **No git changes detected**: "No uncommitted changes found. Make some code changes first, then run @codereview again."
...
### Iterative Review
- After fixes are applied, encourage running @codereview again
...
**With context**:
```
@codereview for the authentication changes I just made
```

**Focused review**:
```
@codereview focusing on security in UserService.java
```

**After fixes**:
```
@codereview - did I fix the issues?
```
